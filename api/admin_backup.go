...
func registerAdminBackupRoutes(router fiber.Router, cfg *config.Config) {
	...
	router.Post("/backup", requireJSON, handler.backup)
	router.Post("/restore/preview", requireJSON, clientIP, handler.preview)
	router.Post("/restore", requireJSON, clientIP, handler.apply)
}
...
func (h *adminBackupHandler) backup(c *fiber.Ctx) error {
	var request backupRequest
	if body := bytes.TrimSpace(c.Body()); len(body) > 0 {
		if err := decodeAdminJSON(body, &request); err != nil {
			return adminError(c, fiber.StatusBadRequest, "invalid request: "+err.Error())
		}
	}
	...
	c.Set(fiber.HeaderContentDisposition, `attachment; filename="`+backup.Filename+`"`)
	c.Set(fiber.HeaderContentType, fiber.MIMEApplicationJSON)
	return c.Status(fiber.StatusOK).Send(backup.Body)
}

func (h *adminBackupHandler) preview(c *fiber.Ctx) error {
...
func (h *adminBackupHandler) apply(c *fiber.Ctx) error {
	request, plaintext, sent, err := h.readRestore(c)
	if sent {
		return err
	}
	if len(request.Fingerprint) == 0 {
		return adminError(c, fiber.StatusBadRequest, "the fingerprint of the preview is required")
	}
...
func (h *adminBackupHandler) readRestore(c *fiber.Ctx) (request *restoreRequest, plaintext []byte, sent bool, err error) {
	body := c.Body()
	if len(body) > adminRestoreMaximumBodySize {
		return nil, nil, true, adminError(c, fiber.StatusRequestEntityTooLarge, "request body is too large")
	}
	request = &restoreRequest{}
	if err := decodeAdminJSON(body, request); err != nil {
		return nil, nil, true, adminError(c, fiber.StatusBadRequest, "invalid request: "+err.Error())
	}
	if len(bytes.TrimSpace(request.File)) == 0 || bytes.Equal(bytes.TrimSpace(request.File), []byte("null")) {
		return nil, nil, true, adminError(c, fiber.StatusBadRequest, "the backup file is required")
	}
	clientIP, _ := c.Locals(security.LocalsClientIP).(netip.Addr)
	now := time.Now()
	if blocked, retryAfter := restorePasswordLimiter.Blocked(clientIP, now); blocked {
		...
		return nil, nil, true, adminError(c, fiber.StatusTooManyRequests, "too many wrong passwords, try again later")
	}
...
func backupError(c *fiber.Ctx, err error) error {
	switch {
	case errors.Is(err, adminbackup.ErrBusy):
		c.Set(fiber.HeaderRetryAfter, derivationRetryAfterSeconds)
		return adminError(c, fiber.StatusTooManyRequests, err.Error())
	case errors.Is(err, adminbackup.ErrTooLarge):
		return adminError(c, fiber.StatusUnprocessableEntity, err.Error())
	case errors.Is(err, adminbackup.ErrFingerprintMismatch):
		return adminError(c, fiber.StatusConflict, err.Error())
	case errors.Is(err, adminbackup.ErrCycleInProgress), errors.Is(err, adminbackup.ErrUnavailable):
		return adminError(c, fiber.StatusServiceUnavailable, err.Error())
	case errors.Is(err, adminbackup.ErrStorageNotSupported):
		return adminError(c, fiber.StatusNotImplemented, err.Error())
	case errors.Is(err, adminbackup.ErrInvalidFile), errors.Is(err, adminbackup.ErrInvalidPassword), errors.Is(err, adminbackup.ErrPasswordRequired),
		...
		return adminError(c, fiber.StatusBadRequest, err.Error())
	default:
		logr.Errorf("[api.backupError] Backup or restore failed: %s", err.Error())
		return adminError(c, fiber.StatusInternalServerError, "the backup or the restore failed")
	}
}
