#!/usr/bin/env python3
"""Manages the endpoints of Gatus through the administration API: imports them from a CSV, renames groups and exports
the push tokens.
...
Examples:
    ...
    python3 docs/manager-gatus.py csv --csv inventory.csv --out endpoints-prod.csv
    ...
    python3 docs/manager-gatus.py export-tokens --out hosts-tokens.csv

The address and the credentials come from --gatus-url, --username and --password, or from GATUS_URL, GATUS_USERNAME
and GATUS_PASSWORD, which keeps the password out of the shell history. Only the standard library is used.
"""
...
class GatusError(Exception):
    """An answer of Gatus, or a file, that the script cannot go on with"""
...
class Gatus:
    ...
    def __init__(self, arguments: argparse.Namespace) -> None:
        if not arguments.password:
            raise GatusError("the password of the administration is required (--password or GATUS_PASSWORD)")
        self.base_url = arguments.gatus_url.rstrip("/")
    ...
    def list_endpoints(self) -> list[dict]:
        ...
        if status != 200 or not isinstance(body, list):
            raise GatusError(f"the list of endpoints answered {status}: {describe(body)}")
        return body
...
def command_import(arguments: argparse.Namespace) -> int:
    rows = filtered_rows(arguments)
    gatus = None if arguments.dry_run else Gatus(arguments)
    existing = {item.get("key", "") for item in gatus.list_endpoints()} if gatus else set()
    print(f"{len(rows)} endpoints in {arguments.csv}" + (" (dry run)" if gatus is None else f" -> {arguments.gatus_url}"))
...
def command_groups(arguments: argparse.Namespace) -> int:
    ...
    for item in Gatus(arguments).list_endpoints():
...
def command_rename_group(arguments: argparse.Namespace) -> int:
    ...
    for item in managed:
        ...
        status, answer = gatus.update_endpoint(key, definition, version)
        if status == 200:
            renamed += 1
            if arguments.verbose:
                print(f"  ~ {key} -> {new_key}")
...
def command_export_tokens(arguments: argparse.Namespace) -> int:
    ...
    for item in gatus.list_endpoints():
        if not item.get("acceptsPush"):
            continue
        token = gatus.get_endpoint(item["key"]).get("pushToken") or ""
        if not token:
...
def filtered_rows(arguments: argparse.Namespace) -> list[dict[str, str]]:
    ...
    if not rows:
        raise GatusError("no endpoint left after the filters")
    return rows
...
def parse_arguments(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Manages the endpoints of Gatus: imports them from a CSV, renames groups and exports the push tokens",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    subcommands = parser.add_subparsers(dest="command", required=True)

    csv_parser = subcommands.add_parser("csv", help="rewrites an inventory of hostnames as the simple CSV, without calling Gatus")
