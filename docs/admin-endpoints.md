...
## Importing endpoints from a CSV

[docs/import-endpoints-from-csv.py](import-endpoints-from-csv.py) registers a list of hosts through this API, one HTTP
endpoint per host, each one also accepting [push](push-monitoring.md#push-on-active-endpoints) with a token of its own.
...
simple format the script itself writes, with the columns `grupo`, `nome` and `url`.

```bash
# Rewrites the inventory as the simple CSV, to review what is going to be registered
python3 docs/import-endpoints-from-csv.py --csv inventory.csv --write-csv endpoints-prod.csv

# Shows what would be registered, without calling Gatus
python3 docs/import-endpoints-from-csv.py --csv endpoints-prod.csv --dry-run

# Registers for real
export GATUS_PASSWORD='your-password'
python3 docs/import-endpoints-from-csv.py --csv endpoints-prod.csv \
  --gatus-url https://status.example.com --username admin

# Exports host,token of every endpoint that accepts push
python3 docs/import-endpoints-from-csv.py --gatus-url https://status.example.com \
  --username admin --export-tokens hosts-tokens.csv
```

| Flag | What it does |
|------|--------------|
| `--csv` | The CSV to read. |
| `--write-csv` | Only writes the filtered CSV in the simple format and exits, without calling Gatus. |
| `--export-tokens` | Only exports `host,token` of the endpoints that accept push, in a CSV. |
| `--gatus-url`, `--username`, `--password` | Address and credentials of the administration, also read from `GATUS_URL`, `GATUS_USERNAME` and `GATUS_PASSWORD`. |
...
| `--limit`, `--dry-run`, `--verbose`, `--timeout`, `--insecure` | Registers at most N, shows without sending, prints one line per endpoint, changes the timeout, skips the TLS verification. |

Running it again only creates what is missing: an endpoint whose key already exists is left untouched, so an
