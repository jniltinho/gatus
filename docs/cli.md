...
The `gatus` binary has a command line. Without a command it starts the server, exactly as before, so the image, the
systemd units and everyone who runs `./gatus` keep working without any change.

```text
gatus                       start the server (same as `gatus serve`)
gatus serve                 start the server
gatus version               version, commit and build date
gatus config validate       validate the configuration without starting anything
gatus password hash         hash a password for security.basic and for the login of a status page
gatus healthcheck           check /health of the local server, for the HEALTHCHECK of the image
gatus completion <shell>    shell completion (bash, zsh, fish, powershell)
```
...
|------|-------------|---------|
| `--config` | `GATUS_CONFIG_PATH` (and the deprecated `GATUS_CONFIG_FILE`) | `config/config.yaml`, then `config/config.yml` |
| `--log-level` | `GATUS_LOG_LEVEL` | `INFO` |

The flag wins over the environment, and the environment over the default. Both flags work on every command, before or
after it: `gatus --config x.yaml` is `gatus serve --config x.yaml`. `GATUS_DELAY_START_SECONDS` still delays the start
...
- **A path given with `--config` must exist.** Through the environment, a missing path falls back on the default paths,
  as it always did; typed on the command line, it is an error — otherwise `gatus config validate --config typo.yaml`
...
- **A reload uses the same path the server started with.** The configuration is checked every 30 seconds, and the file
  that is loaded again is the one of `--config`, even when `GATUS_CONFIG_PATH` points elsewhere.

## gatus config validate
...
```bash
gatus config validate --config ./config.yaml && docker compose up -d
```
...
```bash
gatus password hash                          # asks for the password
printf 'the-password' | gatus password hash  # from a pipe
```
...
```text
$ gatus version
gatus 5.36.0-fork.28 (commit: 46681c7f, built: 2026-09-19T08:13:45Z)
```
