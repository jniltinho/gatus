...
```console
docker run -p 8080:8080 --name gatus jniltinho/gatus:v6.1.0
```
...
| `ui.logo`                 | URL to the logo to display in the header and on the login pages. No logo is shown when empty.                                            | `""`                                                |
...
| `ui.custom-css`           | Custom CSS. Loaded before the stylesheet of the application, so rules that override it (for instance the thin scrollbar of the interface, or `body { font-variant-numeric: normal !important }` to turn off tabular figures) need `!important`. Changing the font family does not: `body { font-family: "Your font", sans-serif }` is enough. | `""`                                                |
| `ui.dark-mode`            | Whether to enable dark mode by default. The theme chosen with the theme button is remembered in the browser and takes precedence.        | `true`                                              |
...
command:
```console
docker run -p 8080:8080 --mount type=bind,source="$(pwd)"/config.yaml,target=/config/config.yaml --name gatus jniltinho/gatus:v6.1.0
```

If you're on Windows, replace `"$(pwd)"` by the absolute path to your current directory, e.g.:
```console
docker run -p 8080:8080 --mount type=bind,source=C:/Users/Chris/Desktop/config.yaml,target=/config/config.yaml --name gatus jniltinho/gatus:v6.1.0
```
...
image: `--set image.repository=jniltinho/gatus --set image.tag=v6.1.0`.
