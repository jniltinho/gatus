...
```console
docker run -p 8080:8080 --name gatus jniltinho/gatus:v6.0.2
```
...
command:
```console
docker run -p 8080:8080 --mount type=bind,source="$(pwd)"/config.yaml,target=/config/config.yaml --name gatus jniltinho/gatus:v6.0.2
```

If you're on Windows, replace `"$(pwd)"` by the absolute path to your current directory, e.g.:
```console
docker run -p 8080:8080 --mount type=bind,source=C:/Users/Chris/Desktop/config.yaml,target=/config/config.yaml --name gatus jniltinho/gatus:v6.0.2
```
...
image: `--set image.repository=jniltinho/gatus --set image.tag=v6.0.2`.
