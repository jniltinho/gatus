...
password of the administration. In the form you type the password itself and Gatus stores only the hash: editing the
...
- **everything the page serves asks for the credential**: the page at `/status/<slug>`, the details page of each
  ...
  its own dialog (HTTP Basic), and `curl -u client:password https://gatus.example.org/status/clients` works the same;
...
services:
  gatus:
    image: jniltinho/gatus:v5.36.0-fork.26
    ports:
      - "127.0.0.1:8080:8080"
    volumes:
      - ./config:/config:ro
...
- The badges, uptimes and response times by key (`/api/v1/endpoints/<key>/...`) are already public in the original
  Gatus and stay that way, including for the endpoints of a page with a [login of its own](#login-of-a-page).
