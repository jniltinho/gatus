...
docker run -d --name gatus -p 127.0.0.1:8080:8080 -v "$PWD/config:/config" jniltinho/gatus:v6.1.0
```
...
```bash
docker run --rm -it jniltinho/gatus:v6.1.0 password hash             # asks for the password, prints the hash
docker run --rm -v "$PWD/config:/config:ro" jniltinho/gatus:v6.1.0 config validate   # before every deploy
docker compose up -d
```
...
```bash
tar xzf gatus_6.1.0_linux_amd64.tar.gz
