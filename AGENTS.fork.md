...
- Screens are published in [docs/screenshots](docs/screenshots): curated images, committed to the repository, linked from the `README.md`. They are taken by `docs/screenshots/capture.sh` (after `make build`), which starts a local instance, registers the data through the administration API, lets the history fill and captures the 11 screens at 1280×900 with `agent-browser`; run it again whenever a screen changes, and add the new screen to the script. The logo (`.github/assets/logo.svg`, in blues, and `logo-flat.svg` without gradients) is the source of every icon in `web/app/public/`: the large sizes come from `logo.svg` and the favicons from its flat version, rasterized with a headless Chrome, and `make frontend-build` copies them to `web/static/`.
