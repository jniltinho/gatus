...
func TestNewStore_MySQLSchema(t *testing.T) {
	forEachMySQLTestServer(t, func(t *testing.T, dsn string) {
		...
		_ = rows.Close()
		slices.Sort(tables)
		expectedTables := []string{"endpoint_alerts_triggered", "endpoint_events", "endpoint_result_conditions", "endpoint_result_messages", "endpoint_results", "endpoint_uptimes", "endpoints", "managed_endpoints", "managed_status_pages", "suite_results", "suites"}
