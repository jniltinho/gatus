...
func TestNewStore_MySQLSchema(t *testing.T) {
	forEachMySQLTestServer(t, func(t *testing.T, dsn string) {
		...
		_ = rows.Close()
		slices.Sort(tables)
		expectedTables := []string{"endpoint_alerts_triggered", "endpoint_events", "endpoint_result_conditions", "endpoint_results", "endpoint_uptimes", "endpoints", "managed_endpoints", "managed_status_pages", "suite_results", "suites"}
		...
		if err := store.db.QueryRow(`SELECT COUNT(*) FROM information_schema.referential_constraints WHERE constraint_schema = DATABASE() AND delete_rule = 'CASCADE'`).Scan(&cascadingForeignKeys); err != nil || cascadingForeignKeys != 7 {
			t.Errorf("expected 7 foreign keys with ON DELETE CASCADE, got %d (err=%v)", cascadingForeignKeys, err)
		}
