...
func (s *Store) GetUptimesByKeys(keys []string, now time.Time) (map[string]*common.EndpointUptimes, error) {
	...
	placeholders := make([]string, 0, len(uniqueKeys))
	...
	rows, err := s.db.Query(`
		...
		WHERE u.hour_unix_timestamp >= $3
			...
			AND e.endpoint_key IN (`+strings.Join(placeholders, ", ")+`)
	...
	for rows.Next() {
		var key string
		var total24h, successful24h, total7d, successful7d, total30d, successful30d int64
		if err := rows.Scan(&key, &total24h, &successful24h, &total7d, &successful7d, &total30d, &successful30d); err != nil {
			return nil, err
		}
