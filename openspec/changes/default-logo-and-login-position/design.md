...
`ValidateAndSetDefaults` apara os espaços e troca o valor vazio por `/logo-192x192.png` e `none` por vazio. O frontend não muda de regra: continua mostrando o logo quando a string não é vazia. Assim todas as telas que mostram o logo (dashboard, página pública, login do `security.basic` e o cartão de login do OIDC em `App.vue`) seguem juntas, sem uma segunda cópia da regra em JavaScript.
...
`/logo-192x192.png` é o mesmo caminho que o manifesto (`manifest.json`) já usa para o ícone. Atrás de um proxy com subcaminho o Gatus já não funciona (os demais recursos também são absolutos), então o logo não piora nada.
