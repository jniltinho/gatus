...
- [ ] 1.1 Pacote `liveupdates` (a sequência nunca reinicia; `Close` não zera contadores nem sequência):
...
- [ ] 1.4 Limites de 500 conexões no total e 10 por endereço IP (com `trusted-proxies`):
...
- [ ] 2.3 `StatusPageEndpoint.vue`: aviso com atualização silenciosa, canal reaberto no `watch([slug, key])` e `notifyRefreshSucceeded` nas atualizações periódicas.
- [ ] 2.4 `ResponseTimeChart.vue`: prop `refreshKey`, linha recarregada sem spinner (spinner só na primeira carga e na troca de período ou endpoint) no máximo a cada 60 s, contador reiniciado na troca de `endpointKey`, faixas na hora.
