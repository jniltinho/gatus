...
**O que não pode ser alinhado, e por quê.** O payload público não tem host nem data de expiração: `statuspage/payload.go` diz explicitamente que ele não carrega chave, URL, hostname nem endereço, a spec `status-page-highlights` exige a rota pública "sem chave, URL, hostname, erros ou condições", a spec `certificate-expiration` publica só os dias, e `test/e2e/status-pages.sh` reprova o roteiro se o JSON público contiver a chave ou um endereço. Então **host e data do certificado continuam só no dashboard**, registrados como diferença prevista, e esta change não toca no backend nem na API.

### D2 — Cartão do histórico igual nas duas

- **Prop `compact` no `EndpointCard`, opcional e `default: false`.** A grade do dashboard (`Home.vue`) não passa a prop e **não muda**. Com `compact`, e só com ela: o cartão não repete nome, grupo, host e badge de estado (já estão no cabeçalho da página), as barras vão para 20 px, o realce `hover:scale-[1.01]` é desligado (hoje o cartão inteiro pula na tela de detalhes) e o tooltip é desenhado pelo próprio componente, sem emitir `showTooltip` para o `App.vue` — senão apareceriam dois tooltips.
