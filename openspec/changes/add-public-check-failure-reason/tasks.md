...
- [ ] 3.1 `summary` no payload da página (`statuspage/payload.go`), com `total`, `up`, `down`, `pending` e `unknown`, contando destaques e os endpoints cortados por uma página truncada, e ausente no payload de detalhes.
- [ ] 3.2 Faixa (`web/app/src/components/public/StatusSummary.vue`) mostrando `<n> up` e `<n> down` sempre, e pendentes e sem dados só quando houver, legível nos dois temas e sem rolagem horizontal a partir de 360 px.
- [ ] 3.3 Testes Go do `summary`, inclusive numa página truncada, e atualização do struct de `TestBuildPayload_Allowlist` para o campo novo.
- [ ] 3.4 E2E: a faixa da página `services` mostra a contagem com o endpoint `offline` fora.

## 4. Documentação e entrega

- [ ] 4.1 `docs/status-pages.md` (a frase sobre o que é publicado e a seção de `show-messages`) e `docs/push-monitoring.md`: tabela dos cinco motivos, a nota de que são constantes estáveis em inglês, e que o erro completo continua só no dashboard, inclusive para páginas com `auth`.
- [ ] 4.2 E2E em `test/e2e/status-pages.sh`: estender a verificação do endpoint `offline` (`http://127.0.0.1:1/`) na página `messages`, exigindo `"message":"Connection failed"` e mantendo a asserção que prova a ausência de `127.0.0.1`, `connection refused` e `dial tcp` no payload.
