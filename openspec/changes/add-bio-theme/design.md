...
- **Os cinzas de estado saem da escala antes de ela ser tingida.** "Sem dados" e "pulado" passam a usar tokens de estado próprios (`status-unknown`, `status-skipped`, com as variantes de borda e de texto que existem hoje), definidos com os valores literais atuais (`#9ca3af`, `#6b7280`, `#4b5563`) e **iguais nos três temas**. A troca é feita ainda na etapa 1, sob a D4 (nenhuma cor computada muda), e o tema `bio` ganha um teste de que as cores de estado são idênticas às do tema claro.
