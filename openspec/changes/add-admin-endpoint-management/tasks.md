...
- [x] 1.9 Abrir o pull request no fork, deixar o CI verde e fazer merge (os cinco marcos entraram juntos no PR #1)
- [x] 1.10 Publicar `v5.36.0-fork.1` pela skill `create-release`: tag e release dos binários pelo workflow, imagem `jniltinho/gatus:v5.36.0-fork.1` com `make docker-release`; conferir release, tarballs e imagem
...
- [x] 2.7 Pull request, CI verde e merge
...
- [x] 3.8 Pull request, CI verde e merge
...
- [x] 4.2 Tipo `ManagedEndpoint` (com `version`) e interface `ManagedEndpointStore`; tabela `managed_endpoints` em SQLite e PostgreSQL com erro no `CREATE`; implementação com transações e conflito de unicidade como erro próprio; testes em SQLite e PostgreSQL (`GATUS_TEST_POSTGRES_URL`) e serviço PostgreSQL no `ci.yml`
- [x] 4.3 Remoção dos dados de uma chave no store SQL, na mesma transação da remoção do endpoint gerenciado (a administração exige SQLite ou PostgreSQL), invalidando o `writeThroughCache` da chave; testes
- [x] 4.4 Validação dos gerenciados: decodificação estrita sem env, campos bloqueados, `client.tunnel`, validação estrita de alertas por endpoint, chave única por `Key()` com origem na mensagem, `extra-labels` contidos na lista registrada; padrões só em memória; serialização `definition` e `effective`; testes
...
- [x] 4.9 Pull request, CI verde e merge
...
- [x] 5.4 View de formulário: modo formulário e modo YAML, conversão com serializador próprio e `POST /parse`, nome e grupo somente leitura, segredos mascarados, tratamento de 412
...
- [x] 5.7 Roteiro `test/e2e/admin.sh` com `agent-browser` (lista, criar, validar, testar, salvar, editar, desabilitar, remover, sem credenciais) nos temas claro e escuro, com capturas em `dist/prints/`
- [x] 5.8 Documentação em `docs/admin-endpoints.md`, com uma seção curta no `README.md` apontando para ela: exemplo de `admin`, pré-requisitos, API, segredos mascarados, várias instâncias, ordenação das tags do fork e rollback
- [x] 5.9 `openspec validate add-admin-endpoint-management --strict`, `make lint`, `make test`, roteiro E2E e CI verdes
- [x] 5.10 Pull request e merge; publicar a próxima release pela skill `create-release` e conferir a imagem no Docker Hub (`v5.36.0-fork.1`)
