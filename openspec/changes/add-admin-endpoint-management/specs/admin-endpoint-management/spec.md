...
A alteração de um endpoint gerenciado MUST NOT mudar sua chave: qualquer diferença no texto de `name` ou `group` em relação ao armazenado MUST ser rejeitada com 400.

#### Scenario: Tentativa de renomear
- **WHEN** um administrador envia `PUT /api/v1/admin/endpoints/core_api` com `name` diferente do armazenado
- **THEN** a API responde 400
- **AND** o endpoint permanece inalterado
