...
- **Presos ao `fasthttp`:** `SetBodyStreamWriter` (3 usos, `internal/api/live_updates.go`), `c.Context().RemoteIP()` (8), `server.HeaderReceived` com `fasthttp.RequestConfig` (`internal/controller/controller.go`), `fiber.Config{ReadBufferSize, Network, Immutable}` e a opção pública `web.read-buffer-size`.
- **APIs de mesmo nome e significado diferente**, conferidas no código do Echo v5.3.1: `c.Path()` no Fiber é o caminho **da requisição**, e no Echo é o padrão **registrado** (`/status/:slug`) — 3 usos nossos, entre eles `statusPageHTMLAuth` (`internal/api/status_page_auth.go:140`), o skipper da compressão (`internal/api/api.go:70`) e o limite do restore (`internal/api/admin_middleware.go:45`). `c.Get(...)` no Fiber lê um **cabeçalho** (11 usos: `Origin`, `Sec-Fetch-Site`, `Authorization`, `If-Match`, `Last-Event-ID`...), e no Echo lê o **store** da requisição. `c.Body()` (12 usos) não existe no Echo: o corpo é um `io.ReadCloser` que só se lê uma vez, e `adminRequestProtection` o lê **antes** do manipulador.
...
- **O Fiber deixa mudar a resposta depois do manipulador; o `net/http` não.** `statusPageBadgeHandler` (`internal/api/status_page_auth.go:125-131`) chama o gerador do badge e **só depois** grava `Cache-Control: private, no-store` e `Vary: Authorization`; `internal/api/spa.go:24` troca a resposta por um erro depois de começar a escrever o template. No `net/http`, cabeçalho gravado depois do primeiro byte não chega ao cliente.
- `controller.Handle` trata **qualquer** erro do `Listen` como fatal (`internal/controller/controller.go:40,45`). O Fiber volta sem erro num desligamento; `http.Server` volta `http.ErrServerClosed`.
- O login do OIDC (`internal/security/oidc.go:59`) recebe `*fiber.Ctx`, grava dois cookies e redireciona; só o **callback** já é `net/http`.
...
- **A ordem de registro é semântica no Fiber.** `internal/api/api.go` diz: *"ORDER IS IMPORTANT: all routes applied AFTER the security middleware will require authn"*. As rotas públicas, o SPA e o `filesystem` de captura são registrados **antes** do middleware de segurança; as protegidas, depois.
...
| Resposta depois do manipulador | Cabeçalho e corpo podem mudar até o fim | Cabeçalho gravado depois do primeiro byte **se perde** | Nenhum código grava cabeçalho depois de escrever. `statusPageBadgeHandler` passa a definir `Cache-Control` e `Vary` **antes** de chamar o gerador, e os geradores de badge deixam de sobrescrever um `Cache-Control` já definido. `internal/api/spa.go` renderiza o template num buffer, para poder trocar a resposta por um erro. O `HTTPErrorHandler` MUST respeitar resposta já iniciada — o canal de eventos sobretudo. |
...
`internal/api/live_updates.go` deixa de usar `SetBodyStreamWriter`: o manipulador escreve direto em `c.Response()` e descarrega com `http.NewResponseController(...).Flush()`. Consequências:
...
- `internal/security/admin.go` e `internal/security/config.go` deixam de converter a requisição (`adaptor.ConvertRequest`): usam `c.Request()` direto.

### D7 — Todo o código Go vai para `internal/`

`internal/` é o que a documentação do Go recomenda para o código que ninguém de fora deve importar, e é o padrão dos projetos do dono (`go-ispconfig`): `main.go` na raiz, os comandos em `cmd/` e o resto em `internal/`.

A primeira versão desta decisão movia só os pacotes do fork, para não transformar cada sincronização com o upstream em conflito em massa. **O dono decidiu em 2026-09-19 que o projeto deixa de ser fork**: nada é sincronizado com o TwiN/gatus nem enviado a ele. Sem sincronização, o argumento cai, e o marco 3 move **todos** os pacotes: `adminbackup`, `alerting`, `api`, `client`, `config`, `controller`, `jsonpath`, `lifecycle`, `liveupdates`, `managedendpoint`, `metrics`, `pattern`, `push`, `pushkey`, `security`, `statuspage`, `storage` e `watchdog`, além de `httpx`, que já nasceu lá.

Ficam fora de `internal/`:

- `main.go` e `cmd/`, por definição;
- `web/` (pacote `static`), porque o `//go:embed` precisa estar ao lado de `web/static/`, que o `make frontend-build` gera;
- `third_party/`, que o `replace` do `go.mod` aponta por caminho;
- `test/e2e/` e `docs/`, que não são código Go. `test/mock.go` vai para `internal/test/`.

`testdata/` acompanha os pacotes (`internal/testdata/`), para que os caminhos relativos `../testdata` dos testes continuem valendo. A mudança é só de caminho: `git mv` e a troca do prefixo de importação, sem alteração de conteúdo, para o git seguir cada arquivo como renomeado. Com o código todo do projeto, `make lint` e `make fmt` deixam de se restringir aos arquivos alterados desde `UPSTREAM_BASE` e passam a cobrir todos os `.go`, menos `third_party/`.

O módulo continua `gatus/v5` mesmo com as tags passando a `v6.0.0` (o dono decidiu, na mesma data, abandonar o sufixo `-fork.N`): como nada fora de `internal/` é importável, a regra do Go que amarra o caminho do módulo à versão maior não se aplica a ninguém.
...
- ~~`internal/` para tudo, como no `go-ispconfig`, ou só para os pacotes do fork?~~ **Decidido pelo dono em 2026-09-19: tudo em `internal/`**, já que o projeto deixa de ser fork (D7).
