...
#### Scenario: DSN válida
- **WHEN** a configuração tem `storage.type: mysql` e `storage.path: "gatus:segredo@tcp(mariadb:3306)/gatus"`
- **THEN** a configuração é válida e o Gatus inicia com o storage MySQL
...
- **WHEN** `storage.path` é `gatus:segredo@tcp(mariadb:3306` (DSN malformada)
- **THEN** a validação falha
- **AND** nem o erro nem o log contêm `segredo`

#### Scenario: Servidor abaixo da versão mínima
- **WHEN** o Gatus inicia com `mysql` conectado a um MySQL 8.0
...
- **WHEN** o Gatus inicia com `mysql` num banco vazio
- **THEN** todas as tabelas existem ao final e o monitoramento grava resultados

#### Scenario: Reinício
- **WHEN** o Gatus reinicia sobre um banco MySQL já inicializado e com resultados
...
A suíte de conformidade do store SQL e os testes dos helpers do fork (endpoints gerenciados, status pages gerenciadas e leituras em lote) MUST rodar em MySQL quando `GATUS_TEST_MYSQL_URL` estiver definido e em MariaDB quando `GATUS_TEST_MARIADB_URL` estiver definido, cada teste num banco próprio criado e removido pelo helper. O CI MUST executar esses testes nas versões mínimas (MySQL 8.4 e MariaDB 10.11) e nas LTS mais novas. Sem as variáveis, os testes MUST continuar passando só com SQLite (e PostgreSQL, se configurado).

#### Scenario: Execução local sem MySQL
- **WHEN** `go test ./...` roda sem `GATUS_TEST_MYSQL_URL` e sem `GATUS_TEST_MARIADB_URL`
