...
- [ ] 1.1 Esquema `endpoint_response_time_buckets` (chave única no `CREATE TABLE`, `ON DELETE CASCADE`) em SQLite, PostgreSQL e MySQL/MariaDB, com `mysql_schema_test.go` atualizado (tabela nova e 9 FKs em cascata).
- [ ] 1.2 SQL:
...
- [ ] 1.4 Interface opcional `ResponseTimeChartReader` (`GetResponseTimeBuckets` e `GetRecentResponseTimeResults`):
  - ordem crescente;
...
- [ ] 2.1 `endpointIntervalSeconds(cfg, key)`: arquivo, depois external do arquivo, depois gerenciado (ativo ou push); nulo quando é 0, em conflito ou inválido. Testes com os cinco tipos.
- [ ] 2.2 Rota protegida `GET /api/v1/endpoints/{key}/response-time-chart?period=`:
  ...
  - `no-store`.
- [ ] 2.3 Rota pública `GET /api/v1/status-pages/{slug}/endpoints/{key}/response-time-chart?period=`:
...
- [ ] 3.1 `utils/responseTimeChart.js`, com testes em `node --test`:
  - `recentDatasets`;
  - `bucketDatasets`:
...
- [ ] 3.2 `ResponseTimeChart.vue` reescrito:
...
- [ ] 3.3 `EndpointDetails.vue` e `StatusPageEndpoint.vue`: seletor Recent/3h/6h/24h/1w no cabeçalho do cartão, rotas protegida e pública, sem `events`, `results` e `duration` no gráfico, com os badges sem mudança.
- [ ] 3.4 Remover `utils/downtime.js` e seus testes, `chartjs-plugin-annotation` (`package.json` e `package-lock.json`) e `RESPONSE_TIME_DURATIONS`.
- [ ] 3.5 Lint, `npm run test:unit` e `make frontend-build`.
...
- [ ] 4.3 `go test ./... -race` com PostgreSQL, MySQL e MariaDB, `make lint` e `openspec validate kuma-response-time-chart --strict`.
- [ ] 4.4 Entrega:
