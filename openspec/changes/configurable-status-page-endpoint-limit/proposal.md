...
3. **O limite de exibição continua decidindo o acesso.** A seleção (`statuspage.Select`) é a mesma que autoriza as rotas por endpoint de uma página: um endpoint além do corte responde 404 nas APIs de detalhes, do gráfico e do stream de eventos e nos badges, como hoje além dos 200 (o HTML continua entregando a SPA, e as páginas com login continuam respondendo 401 antes). O limite viaja com a captura da página (`Published`), e a change conserta os caminhos que hoje resolvem a página mais de uma vez — o gráfico, em especial — para que payload, autorização e contagens enxerguem o mesmo valor.
...
- `status-page-highlights`: os destaques contam no limite em vigor, no lugar de "200".
