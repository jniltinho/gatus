...
- [x] 1.3 `security`: construtor exportado de limitador de falhas independente com a janela como parâmetro (restore com 15 minutos, login com 1 minuto), e testes de que um não afeta o outro.
...
- [x] 2.5 Rotas `POST /api/v1/admin/backup`, `/restore/preview` e `/restore`:
...
- [x] 2.6 Testes de API e integração:
...
- [x] 3.1 Utilitários, com testes em `node --test`:
  ...
  - `adminApi.download`.
- [x] 3.2 `views/admin/AdminBackup.vue`, rota `/admin/backup` em `router/index.js` (`meta.admin`) e `AdminTabs` com a aba Backup:
...
- [x] 4.1 Documentação:
  ...
  - `AGENTS.fork.md`.
- [x] 4.2 E2E `test/e2e/admin-backup.sh`:
...
- [x] 4.3 `go test ./... -race` com PostgreSQL, MySQL e MariaDB, `make lint` e `openspec validate add-admin-backup-restore --strict`.
- [x] 4.4 Entrega:
